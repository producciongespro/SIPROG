...
$mysqli=conectarDB();
	...
	function registraUsuario($pass_hash, $nombre, $apellido1, $apellido2, $instancia, $email, $activo, $token, $tipo_usuario){
		global $mysqli;
		$stmt = $mysqli->prepare("INSERT INTO usuarios (password, nombre, apellido1, apellido2, id_area, correo, activacion, token, id_tipo) VALUES(?,?,?,?,?,?,?,?,?)");
		$stmt->bind_param('ssssisisi', $pass_hash, $nombre, $apellido1, $apellido2, $instancia, $email, $activo, $token, $tipo_usuario);
	...
	function login($usuario, $password)
	{
		...
		$stmt = $mysqli->prepare("SELECT id, id_tipo, password FROM usuarios WHERE  correo = ? LIMIT 1");
		$stmt->bind_param("s", $usuario);
		$stmt->execute();
		$stmt->store_result();
		$rows = $stmt->num_rows;
